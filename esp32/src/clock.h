...
// UTC wall clock. Sources, in order of trust: NTP (when the Wi-Fi has internet),
// then the LX200 client (:SG/:SL/:SC) or the dashboard's browser time. Later: GPS.
void clockBegin();
bool clockValid();
double clockNow();  // unix seconds, UTC, with sub-second resolution
const char *clockSource();

// Sets the clock unless NTP is in charge. Returns true if applied.
