...
$("ha").textContent=s.axis_ha.toFixed(4)+"° / reg "+s.register_deg.toFixed(2)+"°"+(s.position_trusted?"":"  NOT SYNCED");
...
static void sendStatus() {
  ...
  snprintf(buf, sizeof(buf),
           ...
           "\"axis_ra\":%.6f,\"axis_ha\":%.6f,\"counts\":%ld,\"counts_ms\":%lu,\"register_deg\":%.4f,\"ra_shift\":%.4f,\"position_trusted\":%s,\"ra_target\":%.6f,\"lst\":%.6f,\"lst_hms\":\"%s\","
           ...
           r.axisRa, r.axisHa, r.counts, (unsigned long)r.countsMs, r.registerDeg, r.registerDeg - r.axisHa,
           mount::positionTrusted() ? "true" : "false", l.raTarget, lst, lstHms, clockNow(), clockValid() ? "true" : "false",
...
static void postSettings() {
  ...
  if (srv->hasArg("ra_west_minutes") && srv->arg("ra_west_minutes").length())
    settings.raWestMinutes = constrain(srv->arg("ra_west_minutes").toInt(), 0, 240);
